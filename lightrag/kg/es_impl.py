...
class ESKVStorage(BaseKVStorage):
    ...
    async def delete(self, ids: List[str]) -> None:
        ...
        try:
            successes, errors = await async_bulk(self.es_client, actions, refresh="wait_for")
            logger.info(f"Deleted {len(successes)} documents, {len(errors)} failed")
            if errors:
    ...
    async def drop(self) -> Dict[str, str]:
        ...
        try:
            ...
            return {"status": "success", "message": f"Deleted {response['deleted']} documents"}
        except Exception as e:
...
class ESVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        if len(embeddings) != len(data):
            raise ValueError(f"Embedding count ({len(embeddings)}) does not match data count ({len(data)})")
        ...
        try:
            success, errors = await async_bulk(self.es_client, actions, refresh="wait_for")
            if errors:
                logger.error(f"Upsert failed for {len(errors)} documents: {errors}")
        except BulkIndexError as e:
